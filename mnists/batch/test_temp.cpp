...
void initialize(float Delta_w01[N_INPUT][N_H1],
      float Delta_b1[N_H1],
      float Delta_w12[N_H1][N_H2],
      float Delta_b2[N_H2],
      float Delta_w23[N_H2][N_OUTPUT],
...
void standlize(float data[N_INPUT],std::vector<float> train_data) {
   for (int j=0;j<28*28;j++) {
      data[j] = train_data[j]/255.0f;
   }
}
void conv_one_hot(float teacher_buf[N_OUTPUT],float label_data) {
...
int main(){
   ...
   initialize(Delta_w01,Delta_b1,Delta_w12,Delta_b2,Delta_w23,Delta_b3);
   ...
   for (int e_counter=0;e_counter<epoch;e_counter++) {
      ...
      for (int b_counter=0;b_counter<batch_size;b_counter++) {
         ...
         // standlize
         standlize(data,train_data[random_value]);
         // convert teacher to one-hot expression
         conv_one_hot(teacher_buf,label_data[random_value]);

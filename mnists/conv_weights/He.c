...
int main(){
	...
	for (int i=0;i<N_INPUT;i++) {
		for (int j=0;j<N_H1;j++)
			w01[i*N_H1+j] = rand_normal(mu,sigma);
	...
	for (int i=0;i<N_H1;i++) {
		for (int j=0;j<N_H2;j++)
			w12[i*N_H2+j] = rand_normal(mu,sigma);
	...
	for (int i=0;i<N_H2;i++) {
		for (int j=0;j<N_OUTPUT;j++)
			w23[i*N_OUTPUT+j] = rand_normal(mu,sigma);
	...
	new_write_weights(save_file,w01,b1,w12,b2,w23,b3);
	//new_get_weights(save_file,w01,b1,w12,b2,w23,b3);
